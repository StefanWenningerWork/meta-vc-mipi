...
get_entity_name() {
    local entities=$(media-ctl -d ${media0} -p | grep -oP "(?<=[0-9]: )vc-mipi-cam [0-9]{1,2}-00(1a|60)")
...
activate() {
    ...
    if cat /proc/device-tree/compatible | grep -q "variscite"; then
        fw_setenv fdt_file imx8mp-var-dart-dt8mcustomboard-vc-mipi-${2}-csi0-csi1.dtb
    fi
    if cat /proc/device-tree/compatible | grep -q "toradex"; then
